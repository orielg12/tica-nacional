...
export default function Results() {
  ...
  const [selectedDate, setSelectedDate] = useState(getPanamaLocalISODate());
  ...
  const loadWinners = async (dateParam?: string) => {
    const dateToFetch = dateParam || selectedDateRef.current;
    setIsFetching(true);
    try {
      if (store.lotteriesMaster.length === 0) {
      ...
      const winners = await fetchPendingWinners(vendorId, dateToFetch);
  ...
  useEffect(() => {
    ...
    const triggerDebouncedLoad = () => {
      ...
      debounceTimer = setTimeout(() => {
        loadWinners(selectedDateRef.current);
      }, 500);
    };

    // Supabase Realtime live sync for instant prize updates
    const channel = supabase.channel('vendor-results-live')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'results' }, triggerDebouncedLoad)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'tickets' }, triggerDebouncedLoad)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'payouts' }, triggerDebouncedLoad)
      .subscribe();
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full overflow-hidden bg-slate-950">
      ...
      <div className="flex-none p-4 pb-2">
        ...
        <p className="text-gray-400 text-xs mb-2">Tickets ganadores pendientes de cobro.</p>

        {/* Selector de Fecha */}
        <div className="flex items-center justify-between gap-2 mb-2.5 bg-slate-900/90 px-3 py-2 rounded-xl border border-slate-800">
          <div className="flex items-center gap-2">
            <Calendar size={16} className="text-teal-400" />
            <span className="text-xs font-bold text-gray-300">Fecha:</span>
            <input
              type="date"
              value={selectedDate}
              onChange={(e) => {
              ...
              className="bg-slate-800 border border-slate-700 text-teal-300 font-bold px-2 py-1 rounded-lg text-xs outline-none focus:border-teal-500"
            />
          </div>
          {selectedDate !== getPanamaLocalISODate() && (
            <button
              onClick={() => {
                const today = getPanamaLocalISODate();
                setSelectedDate(today);
                loadWinners(today);
              }}
              className="px-2.5 py-1 rounded-lg bg-teal-600 active:bg-teal-700 text-white text-xs font-bold transition-colors shadow"
            >
              Ir a Hoy
            </button>
          )}
